...
Para duas doses $a$ e $a_0$, escreva $\mu(a,x)=\Pp\{Y(a)=1\mid X=x\}$. A decomposição
\begin{equation}
\mu(a,x)-\mu(a_0,x)
=\mu(a_0,x)\left\{\frac{\mu(a,x)}{\mu(a_0,x)}-1\right\}
\label{eq:decomp}
\end{equation}
...
\label{sec:setup}
Observamos $O=(X,A,Y)$, com $Y\in\{0,1\}$ e dose $A\in\mathcal A_x\subseteq\R$. As covariáveis $X$ são anteriores à intervenção. Inicialmente, consideramos observações independentes; para simulações repetidas do mesmo cliente, o particionamento e a inferência devem respeitar os clusters.

Assumimos consistência $Y=Y(A)$, ausência de interferência relevante e intercambialidade condicional $Y(a)\perp A\mid X$ para as doses de interesse. Sob essas hipóteses,
\[
\mu(a,x)=\E[Y\mid A=a,X=x].
\]
Para tratamentos contínuos, igualdades condicionais valem quase em toda parte; a identificação em pontos determinados requer uma versão regular, por exemplo contínua em $a$. A interpretação causal depende dessas hipóteses; a identidade de Bayes apresentada adiante, por si só, é apenas probabilística.

Denote por
\begin{align}
f(a\mid x)&=f_{A\mid X}(a\mid x),&
p(x)&=\Pp(Y=1\mid X=x)=\int\mu(u,x)f(u\mid x)\,du,\label{eq:defs}\\
q(a\mid x)&=f_{A\mid X,Y=1}(a\mid x).&&\notag
\end{align}
...
Escolha uma referência pré-especificada $a_0(x)$ no suporte de interesse. Os alvos são
\begin{align}
R(a,a_0;x)&=\frac{\mu(a,x)}{\mu(a_0,x)},&
g(a,a_0;x)&=\log R(a,a_0;x),\label{eq:targets}\\
s(a,x)&=\partial_a\log\mu(a,x).&&\notag
\end{align}
São necessárias conversões de referência estritamente positivas. Quando $A=r$ é a taxa bruta positiva, a elasticidade é $\varepsilon(r,x)=r\,s(r,x)$. Quando $A=\log\{r/r_{\mathrm{base}}(x)\}$, com taxa-base pré-tratamento, $s(a,x)$ já é a elasticidade em relação a $r$. Uma normalização adicional $A=\log\{r/r_{\mathrm{base}}(x)\}/c$ implica $\varepsilon=s/c$.
...
\label{prop:bayes}
Se $p(x)>0$ e $f(a\mid x)>0$ no domínio avaliado, então
\begin{equation}
q(a\mid x)=\frac{\mu(a,x)f(a\mid x)}{p(x)}.
\label{eq:bayes}
\end{equation}
Consequentemente, quando $\mu(a_0,x)>0$,
\begin{equation}
\boxed{R(a,a_0;x)=\frac{q(a\mid x)}{q(a_0\mid x)}
                           \frac{f(a_0\mid x)}{f(a\mid x)}.}
\label{eq:qratio}
\end{equation}
Se as funções são diferenciáveis e positivas em $a$,
\begin{equation}
\boxed{s(a,x)=\partial_a\log q(a\mid x)-\partial_a\log f(a\mid x).}
\label{eq:qderiv}
\end{equation}
\end{theorem}
\begin{proof}
A densidade conjunta de $A=a$ e $Y=1$ dado $X=x$ é $\mu(a,x)f(a\mid x)$. Dividir por $p(x)$ produz \eqref{eq:bayes}. Tomar a razão em duas doses cancela $p(x)$; derivar o logaritmo cancela $\log p(x)$.
\end{proof}
...
Se $\mu(a,x)=b(x)r(a,x)$, com $r(a_0,x)=1$, então
\begin{equation}
q(a\mid x)=\frac{r(a,x)f(a\mid x)}{\int r(u,x)f(u\mid x)\,du}.
\label{eq:tilt}
\end{equation}
O nível $b(x)$ desaparece. Assim, a alteração da distribuição de taxas entre os conversores identifica a \emph{forma relativa} da curva, depois de corrigir a alocação original. Isso não torna o procedimento imune a confundimento: em dados observacionais, a correção por $f(a\mid x)$ continua essencial.
...
Escreva $r_g(a,x)=\exp g(a,x)$ e imponha $g(a_0(x),x)=0$. Para $f$ conhecido, defina
\begin{equation}
q_g(a\mid x)=\frac{f(a\mid x)\exp g(a,x)}{Z_g(x)},\qquad
Z_g(x)=\int f(u\mid x)\exp g(u,x)\,du.
\label{eq:qmodel}
\end{equation}
Um critério natural é
\begin{equation}
\mathcal L_Q(g)=\E\left[Y\{-g(A,X)+\log Z_g(X)\}\right].
\label{eq:qloss}
\end{equation}
...
\end{theorem}
\begin{proof}
Condicionando em $X,Y=1$, a diferença de perdas é a diferença entre entropias cruzadas de $q_0$ e $q_g$. A média na população original pondera esse termo por $p(X)$. Se $q_g=q_0$, \eqref{eq:qmodel} implica que $g-g_0$ é constante em $a$ para cada $x$.
\end{proof}
...
A identidade não obriga a ajustar duas densidades completas. Para cada covariável $X$ de um conversor, podemos contrastar a dose observada, proveniente de $q(\cdot\mid X)$, com doses de referência simuladas de $f(\cdot\mid X)$. Com classes balanceadas \emph{condicionalmente em $X$}, um classificador $D(a,x)$ que distingue as duas origens satisfaz
\[
\frac{D(a,x)}{1-D(a,x)}=\frac{q(a\mid x)}{f(a\mid x)}.
\]
Logo,
\[
g(a,a_0;x)=\logit D(a,x)-\logit D(a_0,x).
\]
A geração de referências deve preservar a distribuição condicional de $A\mid X$. Amostrar taxas marginais de outros clientes sem essa correção é, em geral, incorreto. Referências adicionais reduzem erro de integração ou de contraste, não criam novos conversores independentes. A abordagem pertence à tradição de estimação direta de razões de densidade \cite{sugiyama2012}.
...
\begin{equation}
\mu(a,x)=b_0(x)\exp\{\theta_0^{\mathsf T}H(a,x)\},
\qquad H(a_0(x),x)=0,
\label{eq:structural}
\end{equation}
onde $H:\mathcal A\times\mathcal X\to\R^d$ é uma base conhecida, $d$ é fixo e $b_0(x)=\mu(a_0(x),x)$. O modelo admite interações, splines de dose e não linearidade em $X$ incorporadas a $H$, mas não é uma superfície completamente irrestrita.

Por exemplo, $H(a,x)=a\,v(x)$ com referência zero produz $g(a,x)=a\,v(x)^{\mathsf T}\theta_0$. Se $a$ é a log-variação da taxa, $v(x)^{\mathsf T}\theta_0$ é a elasticidade constante ao longo da janela de dose, mas heterogênea entre perfis.

\subsection{O momento e a identidade de viés}
Defina
\begin{equation}
m_0(x)=\E[H(A,X)\mid X=x].
\label{eq:mH}
\end{equation}
Para funções candidatas $b,m$, considere o vetor
\begin{equation}
\boxed{\psi(O;\theta,b,m)=\{H(A,X)-m(X)\}
   \left[Y\exp\{-\theta^{\mathsf T}H(A,X)\}-b(X)\right].}
\label{eq:psi}
\end{equation}
...
\[
\E\left[Y\exp\{-\theta_0^{\mathsf T}H(A,X)\}\mid A,X\right]=b_0(X).
\]
...
\]
\end{proof}
A garantia não exige conhecer a densidade inteira de $A\mid X$, apenas a média condicional da base $H$. Para $H(a,x)=a\,v(x)$, basta estimar $\E[A\mid X]$, pois $m_0(x)=v(x)\E[A\mid X=x]$. Em um experimento com alocação conhecida, essa média pode ser calculada exatamente pelo desenho.
...
\end{corollary}
\begin{proof}
Substitua $b=b_0+t h_b$ e $m=m_0+t h_m$ em \eqref{eq:product}. O resultado é $t^2\E[h_m(X)h_b(X)]$, cuja derivada em $t=0$ é zero. Variações separadas também têm derivada zero.
\end{proof}
O Jacobiano no ponto verdadeiro é
\begin{equation}
J_0=\partial_\theta\E\psi(O;\theta,b_0,m_0)\big|_{\theta_0}
=-\E\left[b_0(X)\Cov\{H(A,X)\mid X\}\right].
\label{eq:jac}
\end{equation}
Se $-J_0$ é definida positiva, há identificação local. Características de $H$ constantes em $A$ não são identificáveis por esse momento; por isso o intercepto de conversão está em $b_0$, não na base de dose. Baixa variação condicional da taxa ou conversão extremamente pequena enfraquece $J_0$ e aumenta a incerteza.
...
\end{equation}
Pode-se estimar $b_0$ ajustando uma regressão flexível de $Y$ em $(A,X)$ e avaliando-a em $a_0(X)$. Outra alternativa é regressar $Y\exp\{-g_{\rm piloto}(A,X)\}$ em $X$, com um piloto consistente obtido em amostra independente ou por particionamento aninhado. A qualidade desse piloto integra o erro final de $\hat b$; ela não pode ser ignorada.
...
Sejam $\pi_j(a\mid x)$, $j\in\{0,1\}$, duas densidades de intervenção conhecidas e fixas em relação à distribuição observada. Podem ser pequenas janelas em torno de taxas de referência ou políticas com aleatorização controlada. Defina
\begin{equation}
\nu_j(x)=\int\pi_j(a\mid x)\mu(a,x)\,da,\qquad
R_\pi(x)=\frac{\nu_1(x)}{\nu_0(x)},\qquad
\gamma_\pi(x)=\log R_\pi(x).
\label{eq:policytargets}
\end{equation}
Exigimos $\nu_0(x)>0$ e suporte: $\pi_j(a\mid x)>0$ apenas onde $f(a\mid x)>0$. Para inferência regular, são necessárias razões $\pi_j/f$ suficientemente controladas e momentos finitos.

A identidade Q também identifica esses alvos:
\begin{equation}
R_\pi(x)=
\frac{\int\pi_1(a\mid x)q(a\mid x)/f(a\mid x)\,da}
     {\int\pi_0(a\mid x)q(a\mid x)/f(a\mid x)\,da}.
\label{eq:policyQ}
\end{equation}
...
Uma política da forma $\pi_\delta(a\mid x)\propto e^{\delta a}f(a\mid x)$ depende da lei observada. As funções de influência abaixo não podem ser aplicadas a esse alvo simplesmente substituindo $\pi$ por $\hat\pi$; a dependência em $f$ introduz termos adicionais. Podemos tratar uma política aprendida em dados externos como fixa na avaliação, mas o alvo passa a ser o valor daquela política congelada.
\end{remark}
...
\begin{equation}
Z_j(O;\hat\mu,\hat f)=
\underbrace{\int\pi_j(a\mid X)\hat\mu(a,X)\,da}_{\hat\nu_j^{\rm OR}(X)}
+\frac{\pi_j(A\mid X)}{\hat f(A\mid X)}\{Y-\hat\mu(A,X)\}.
\label{eq:Z}
\end{equation}
...
\begin{equation}
\E[Z_j\mid X=x]-\nu_j(x)
= B_j(x):=\int\pi_j(a\mid x)
\left\{\frac{f(a\mid x)}{\hat f(a\mid x)}-1\right\}
\{\mu(a,x)-\hat\mu(a,x)\}\,da.
\label{eq:B}
\end{equation}
Logo o sinal tem esperança condicional correta se $\hat f=f$ ou se $\hat\mu=\mu$.
\end{theorem}
\begin{proof}
Condicionar o termo residual de \eqref{eq:Z} em $(A,X)$ produz a integral de $\pi_j f/\hat f\,(\mu-\hat\mu)$. Subtrair $\nu_j$ e agrupar os termos de regressão produz \eqref{eq:B}.
\end{proof}
...
\[
\hat s(a,x)=\hat\theta^{\mathsf T}\partial_a H(a,x).
\]
...
Para um kernel simétrico $K$ de integral um, defina uma intervenção local $\pi_{a,h}(u)=K_h(u-a)$, no interior do suporte. Se $\mu(\cdot,x)$ é duas vezes diferenciável,
\begin{equation}
\nu_{a,h}(x)=\mu(a,x)+\frac{h^2\kappa_2}{2}\partial_a^2\mu(a,x)+o(h^2),
...
\begin{align}
\log\frac{\nu_{a,h}(x)}{\nu_{a_0,h}(x)}
-\log\frac{\mu(a,x)}{\mu(a_0,x)}
=\frac{h^2\kappa_2}{2}
\left\{\frac{\mu''(a,x)}{\mu(a,x)}-
\frac{\mu''(a_0,x)}{\mu(a_0,x)}\right\}+o(h^2).
...
Se a oferta é $r=r_{\rm base}(X)\,c_T$, com $T$ pertencente a poucos braços, a taxa bruta pode ter milhares de valores na base. Entretanto, condicionalmente em $X$, existem apenas os poucos valores $r_{\rm base}(X)c_t$. Não há uma densidade contínua de alocação nesse desenho. Ajustar uma densidade artificial não identifica o efeito de uma taxa nunca oferecida a perfis comparáveis.
...
Uma forma $b(x)\exp g(a,x)$ deve satisfazer
\begin{equation}
0<b(x)\exp g(a,x)<1\quad\text{no domínio utilizado}.
\label{eq:constraint}
\end{equation}
...
Foram executadas 100 replicações independentes de tamanho $n=30\,000$, com semente inicial 20260905. Geramos $X_1\in\{-1,+1\}$ equiprovável e $X_2\sim\operatorname{Unif}[-1,1]$, independentes. A dose $A\in[-1,1]$ segue
\begin{equation}
f(a\mid x)=\frac{\exp\{\alpha(x)a\}}{2M\{\alpha(x)\}},\quad
...
\begin{equation}
\mu(a,x)=c\exp(1{,}1x_1+0{,}7x_2)
\exp\{a(-0{,}35-0{,}20x_1)\}.
\label{eq:simmu}
\end{equation}
O fator $c=0{,}037104282322969306$ foi calibrado por quadratura para obter $\E[Y]=0{,}06$. A conversão-base em $a=0$ varia aproximadamente de 0,61\% a 22,45\%. As probabilidades são verificadas pelo código e permanecem em $(0,1)$ em todo o suporte deste desenho.

A heterogeneidade relativa verdadeira é $\theta_1=-0{,}20$; o termo comum é $\theta_0=-0{,}35$. Como $f$ é exponencialmente inclinada e $\mu$ é log-linear em $a$, a densidade entre conversores tem inclinação $\alpha(x)+\theta_0+\theta_1x_1$. Isso permite ajustar o Q-Learner por verossimilhança sem erro de integração.

\subsection{O que a comparação testa}
O primeiro experimento fixa as nuisances na verdade ou em funções deliberadamente incorretas. No CQ-DML, usamos $b=b_0$ ou $b\equiv0{,}06$, e $m_A=\E[A\mid X]$ ou $m_A\equiv0$. No Q-Learner, usamos a densidade correta ou uma uniforme em $[-1,1]$. Essas condições isolam a identidade de robustez; não representam a dificuldade completa de aprender funções desconhecidas.
...
Uma realização adicional, com semente 20260906, utilizou $n=100\,000$ e três folds. Um regressor de gradient boosting estimou $\E[A\mid X]$. Um classificador de gradient boosting estimou $\mu(a,x)$, avaliado em $a=0$ para produzir $\hat b$. A função de efeito manteve apenas os dois coeficientes especificados em \eqref{eq:simmu}.
...
\[
\E[A\mid z]=\frac{d}{dz}\log M(z)=\coth z-\frac1z,
\]
